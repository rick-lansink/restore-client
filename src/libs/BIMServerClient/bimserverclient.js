...
export default class BimServerClient {
	constructor(baseUrl, oauthtoken = null, notifier = null, translate = null) {
	...
	generateRevisionDownloadUrl(settings) {
		return this.baseUrl + "/download?token=" + this.oauthtoken + (settings.zip ? "&zip=on" : "") + "&topicId=" + settings.topicId;
	}

	generateExtendedDataDownloadUrl(edid) {
		return this.baseUrl + "/download?token=" + this.oauthtoken + "&action=extendeddata&edid=" + edid;
	}
	...
	multiCall(requests, callback, errorCallback, showBusy, showDone, showError, connectWebSocket) {
		if (!this.webSocket.connected && this.oauthtoken != null && connectWebSocket) {
		...
		//    		this.notifier.clear();
		if (this.oauthtoken && this.oauthtoken.length > 0) {
			request.oauthcode = this.oauthtoken
	...
	checkin(topicId, project, comment, file, deserializerOid, progressListener, success, error) {
		...
		formData.append("oauthcode", this.oauthtoken);
